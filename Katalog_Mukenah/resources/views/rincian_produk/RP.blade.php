...
@section('tampilan')
		...
		<td>{{ $r->keterangan }}</td>
		<td>{{ $r->penjelasan }}</td>
		<td><a href="/rp/{{ $r->produk_id }}"><button>Edit</button></a>
        <a href="/HDrinc_pro/{{$r->produk_id}}" onclick="return confirm('Apakah Anda Yakin Menghapus Data?');"><button>Hapus</button></a>
	</td>
		</tr>
		@endforeach
</table>
